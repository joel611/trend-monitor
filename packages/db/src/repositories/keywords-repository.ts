...
import { keywords, type Keyword, type InsertKeyword, type DbClient } from "../index";
...
export class KeywordsRepository {
	...
	async update(
		...
		try {
			await this.db.update(keywords).set(updates).where(eq(keywords.id, id));
		} catch (err) {
			throw new Error(
	...
	// From processor-worker: loads all active keywords
	async findAll(): Promise<Keyword[]> {
		try {
			return await this.db.select().from(keywords).where(eq(keywords.status, "active"));
		} catch (err) {
			throw new Error(
				`Failed to find all keywords: ${err instanceof Error ? err.message : "Unknown error"}`,
			);
		}
	}
}
