...
const StatBox = ({ label, value, change, up, icon: Icon }: any) => (
	<div className="bg-[#0d1117] border border-emerald-900/40 p-5 relative overflow-hidden group hover:border-emerald-500 transition-colors">
		...
		<div className="text-2xl font-bold text-emerald-200 tracking-tighter">{value}</div>
...
const DashboardOverview: React.FC = () => {
	...
	const topKeywords = [...MOCK_KEYWORDS]
		.sort((a, b) => b.mentionsCount7d - a.mentionsCount7d)
		.slice(0, 5);
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex justify-between items-end border-b border-emerald-900/30 pb-4">
				<div>
					<h1 className="text-2xl font-bold text-emerald-400 terminal-glow uppercase">
						Dashboard Core
					</h1>
						...
						{loadingInsights ? (
							...
							<div className="space-y-3">
								...
								<div className="pt-4 border-t border-emerald-900/40">
									<p className="text-[9px] text-emerald-800 font-bold mb-2 uppercase tracking-tighter">
										Significant Signal Clusters:
									</p>
									<div className="flex flex-wrap gap-2">
										{MOCK_KEYWORDS.filter((k) => k.isEmerging).map((k) => (
											<span
												key={k.id}
												className="border border-emerald-900 px-2 py-0.5 text-[10px] bg-emerald-950/20"
											>
												{k.name}
											</span>
										))}
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<div className="bg-[#0d1117] border border-emerald-900/40 p-6">
					<div className="flex items-center justify-between mb-4">
						...
						<Link
							to="/keywords"
							className="text-[10px] text-emerald-700 hover:text-emerald-500 font-bold border-b border-emerald-900"
						>
							EXTRACT_ALL
						</Link>
					</div>
					<div className="divide-y divide-emerald-900/20">
						{topKeywords.map((kw) => (
							<Link
								to={`/keyword/${kw.id}`}
								key={kw.id}
								className="flex items-center justify-between py-3 hover:bg-emerald-950/10 group transition-colors px-2 -mx-2"
							>
								<div className="flex items-center space-x-4">
									<span className="text-[10px] text-emerald-900 font-mono">
										0x{kw.id.padStart(2, "0")}
									</span>
									<div>
										<h4 className="font-bold text-emerald-200 group-hover:text-emerald-400 transition-colors">
											{kw.name}
										</h4>
										<p className="text-[9px] text-emerald-800 font-bold">
											{kw.tags.join(" // ").toUpperCase()}
										</p>
									</div>
								</div>
								<div className="text-right">
									...
									<p
										className={`text-[10px] font-bold ${kw.growthRate > 0 ? "text-emerald-500" : "text-rose-600"}`}
									>
										{kw.growthRate > 0 ? "+" : ""}
										{kw.growthRate}%
									</p>
				...
				<div className="bg-[#0d1117] border border-emerald-900/40 p-6 flex flex-col">
					<h2 className="text-sm font-bold text-emerald-400 uppercase tracking-widest mb-6">
						Vector Distribution
					</h2>
					<div className="flex-1 flex flex-col justify-center space-y-8">
						...
						<div className="h-4 w-full bg-emerald-950/30 flex overflow-hidden border border-emerald-900/50">
							<div
								className="h-full bg-orange-600/60 border-r border-emerald-900/50"
								style={{ width: "42%" }}
							></div>
							<div
								className="h-full bg-blue-600/60 border-r border-emerald-900/50"
								style={{ width: "35%" }}
							></div>
